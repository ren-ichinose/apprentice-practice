...
- アプリケーションの品質が基準をみたしているかを確認する。
- コードを実行した時の挙動を、効率よく確認する。
- コードの意図を明確にし、保守性を高める。
- バグが起きる要因を事前に取り除く。
- 将来のコードの更新によるバグを検知する。
...
- 他のコードの変更によって影響を受ける可能性があるコード。
- セキュリティに関わるコード。
- 機密情報やお金など、重要な事柄に関するコード。
- 主要なユースケースに関わるコード。
- 複雑なロジックを含むコード。
- 例外処理を含むコード。
- 直感的に不安に感じるコード。
...
- 他のテストで既にカバーされているコード。
- 自動生成されていて、変更を加えていないコード。
- フレームワークの設定だけを行うコード。
- バグが起きた時の影響が小さいコード。
- バグが発生する可能性が低い、かつ発生しても致命的ではないコード。

### テストコードを書くときに意識すること

- モレなく、ダブリなく、必要最小限のテスト項目を抽出する。
- 更新系の処理を行う場合は、更新前と更新後の状態を確認する。
- 上から下へ、読みやすいテストコードを書く。
- わかりやすく、簡潔なテストコードを書く。Quest
- テスト用のデータは、ユースケースに沿ったものを用意する。
- コード以外の要因（実行順序・実行環境・時間など）によってテスト結果が変わらないようにする。
- privateメソッドは、直接テストするのではなく、publicメソッドを経由してテストする。

### 単体テストとは

- ユニットテストとも呼ばれる。
- クラスや関数など比較的小さい単位でテストを行う。

### 結合テストとは

- 単体テストをパスした複数のプログラムを組み合わせてテストを行う。

### E2Eテストとは

- エンドツーエンドテストとも呼ばれる。
- システム全体を通してテストを行う。
- 実際に想定されるシナリオをもとにユーザ視点でテストを行う。