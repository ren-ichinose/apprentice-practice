...
### 同時実行制御とは
- 複数のトランザクションが同時にデータベースにアクセスする場合に、データの整合性を保つために使用される機能。

### ACID特性とは
- トランザクション処理において、守るべき約束ごと。
- 原子性(Atomicity)
  - 全て実行される(COMMIT)か、全て実行されない(ROLL BACK)か、どちらかを保証する。
- 一貫性・整合性(Consistency)
  - データベースにあらかじめ設定された制約を満たすことを保証する。
- 独立性(Isolation)
  - トランザクション同士が互いに干渉を受けないことを保証する。
- 永続性・耐久性（Durabillity）
  - トランザクションが終了したら、その時点でのデータが保存されることを保証する。　
  - 一般的な保証方法は、実行記録をログとしてディスクに保存する方法。

### 同時実行制御を行うための主要な方法であるロックとは
- トランザクションがデータを変更する前に、そのデータをロックして他のトランザクションからのアクセスを制限する。
- 占有ロック
  - データがロックされている間、そのデータを読み込むことも、書き込むこともできない。
- 共有ロック
  - データがロックされている間、そのデータを読み込むことはできるが、書き込むことはできない。
- 楽観的ロック
  - 更新直前にロックをかける。
  - あまり使われていない。
- 悲観的ロック
  - 最初からロックをかける。
- デッドロック
  - 複数のトランザクションが互いに相手のロックの解放待ちをしている状態。
  - デッドロックが発生した場合
    - トランザクションを強制終了させるしかない。
    - 強制終了した場合はロールバックが発生する。
  - デッドロックを発生させない工夫
    - 資源をロックする順番をトランザクション間で統一する。